    ...
    public class FLP : MIDI
    {
        ...
        public static MIDI? Load(long id, string filePath, string name, ParallelOptions parallelOptions, Action<int, int> progressCallback)
        {
            try
            {
                ...
                return new FLP(midi, midiMetaEvents, name, id, filePath, TimeSpan.FromSeconds(seconds), midi.TrackCount, noteCount, (ulong)(new FileInfo(filePath).Length), eventCounts, midi.PPQ);
            }
